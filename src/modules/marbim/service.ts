...
const TARGET_NOTES: Record<string, readonly string[]> = {
  ...
  cut_reports: [
    ...
    'dust on it.',
    '',
    'A sheet prints several numbers per size and only ONE of them is being asked for:',
    '',
    '  · the marker RATIO or per-ply figure — how many of that size are in one ply',
    '  · EXPECTED or planned — ratio times plies, worked out before cutting',
    '  · ACTUAL or CUT — what came off the table, which is the only one that counts',
    '',
    'Read the ACTUAL column into `cut`. If the sheet shows only one number per size, that',
    'is the actual. A short row is the point of the whole document: a size that came out',
    'under the plan is exactly what the report exists to record, so never "correct" it up',
    'to the expected figure.',
    '',
    'The lay number is whatever the sheet calls it — "LAY-32", "L-32", "Lay No 32". One lay',
    'is one colour; if the sheet names a colour, read it.',
  ],
  cartons: [
    'This is a PACKING LIST — cartons, what is in each, and what they weigh.',
    '',
    'One entry per CARTON, in `cartons`. A carton holding two sizes has two entries in its',
    'own `contents`, not two cartons. A range in the carton-number column ("1-12") means',
    'twelve identical cartons: write them out individually, because each one is scanned,',
    'weighed and loaded on its own.',
    '',
    'GROSS is the carton with the goods in it; NET is the goods alone. They are different',
    'numbers and the difference is the carton — do not put one in both. Kilograms unless',
    'the paper says otherwise; do not convert from pounds, read what is printed.',
    '',
    'A "TOTAL" or "SAY TOTAL" row is a sum, not a carton. Leave it out.',
  ],
  machines: [
    'This is a photograph of a MACHINE NAMEPLATE — the rivet-on plate on an industrial',
    'sewing machine, usually metal, often oily and at an angle.',
    '',
    'The SERIAL is the field that has to be exact: it is what a spare part and a service',
    'call are ordered against. Read it character for character, keeping letters, dashes',
    'and leading zeros. If part of it is unreadable, leave the whole field out rather than',
    'guessing a character — a wrong serial orders the wrong part.',
    '',
    'MACHINE TYPE is what it does, in the words the floor uses: "single needle lockstitch",',
    '"overlock 5-thread", "flatlock", "bartack", "buttonhole". The plate often gives a',
    'model code that implies the type; give the type in words as well when you can.',
    '',
    'Do not read a line or a location. Which line a machine stands on is where somebody',
    'put it this morning, not something stamped on the plate.',
  ],
  hourly_outputs: [
  ...
  rfqs: [
    'An ENQUIRY is a buyer asking a price, not placing an order. Everything on it is a',
    'request, and half of it is deliberately unsettled — which is why so many fields here',
    'are optional. Leaving one empty is the correct reading of a page that does not say.',
    '',
    'THE STYLE CODE IS ALMOST NEVER THERE. A buyer enquires against their OWN article',
    'number ("NK-90455"); the factory\'s style code does not exist yet — it gets assigned',
    'when the enquiry is quoted. Put the buyer\'s article in `description` if it helps a',
    'reader, and leave `styleCode` empty unless the page states a code the FACTORY uses.',
    'Inventing one attaches the enquiry to a style nobody has made.',
    '',
    '`targetPrice` is the price the buyer is ASKING FOR — "target", "should not exceed",',
    '"our budget is". It is not a price the factory has quoted and not a landed cost. If',
    'the page states no target, leave it empty: a target invented from a comparable order',
    'becomes the number a merchandiser negotiates against.',
    '',
    '`sizeRatio` is PARTS, not pieces — "ratio 1:2:3:2:1" over XS-XL is',
    '{"XS":1,"S":2,"M":3,"L":2,"XL":1}. A grid of actual quantities is not a ratio; if the',
    'enquiry gives quantities per size rather than parts, leave the ratio empty.',
    '',
    'A DELIVERY WINDOW IS NOT A DATE. "last week of January 2027", "Q1", "week 5" — these',
    'are how enquiries state timing, and none of them is a `requestedShipDate`. Leave it',
    'empty rather than picking a day inside the window: the day chosen becomes a date the',
    'factory is measured against. `deadline` is when the QUOTE is due back, which is a',
    'different thing and usually is a real date.',
    '',
    'Do not fill the buyer: no enquiry carries this system\'s id for one.',
  ],
  orders: [
